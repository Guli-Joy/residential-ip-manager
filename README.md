...
├─ scripts/                 # 环境安装、图标生成、预览、构建和在线烟雾脚本
...
`release.yml` 会在 `windows-latest` 上执行测试、Ruff、Pyright、依赖检查和 EXE 构建，随后
...
.\.venv\Scripts\python.exe scripts\render_preview.py `
  --width 1280 --height 800 --output docs\images\dashboard.png
```
